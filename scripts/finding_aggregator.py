...
def parse_agent_output(agent_name: str, output: str) -> List[Finding]:
    ...
    # Pattern 5: Simple bullet with description
    simple_bullet = re.compile(
        r'[-*]\s*\*\*([^*]+)\*\*[\s:]*[`]?([^`:\s]+)[`]?:?(\d+)?[\s:]*[-:]?\s*(.+)'
    )
...
def parse_json_findings(findings_json: List[Dict[str, Any]]) -> List[Finding]:
    ...
    for i, f in enumerate(findings_json):
        try:
            ...
            evidence_refs = f.get('evidence_refs', [source_agent] if source_agent != 'unknown' else [])
            finding = Finding(
...
def apply_filtering(
    ...
    categories['suppressed'] = [
        {'finding': r.finding, 'reason': r.reason}
        for r in suppressed
    ]
...
def format_results(results: Dict[str, Any]) -> str:
    ...
    lines.append(f"- **Filter effectiveness:** {results.get('filter_effectiveness', 0):.1f}%\n")
    ...
    if critical:
        ...
        for ff in critical:
            ...
            lines.append(f"- [{agent}]: {f.description} [{f.file}:{f.line}] (confidence: {ff.filtered_confidence})")
        lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--context-json",
        required=True,
        help="Path to ProjectContext JSON file"
    )
    parser.add_argument(
        "--findings-json",
        help="Path to findings JSON file (structured format)"
    )
    parser.add_argument(
        "--findings-dir",
